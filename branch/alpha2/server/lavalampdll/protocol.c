...
void setnewseq(datapkt_t* ofthis)
{
	int n;
	...
	if (RAND_MAX <0xFF )
	{
		...
		printf("Go code some workaround in setnewseq().\n");
		exit(1);
	}

	for (n=0; n<4; n++)
	{
		ofthis->seqnum = ofthis->seqnum << 8;
		ofthis->seqnum |= ( (char)rand() & 0x000000FF ) ;
	}	
}
...
generic_packet_response_t* sendPacketTimed(appConfig_t* myconfig, datapkt_t tosend)
{
	...
	response = sendPacket(myconfig,tosend);			// do all the sending/reciving needed for this pkt
	endtime = timeGetTime();						// 
...
generic_packet_response_t* sendPacket(appConfig_t* myconfig, datapkt_t tosend)
{
	...
	if (myconfig->verbose>2) { printf("Packet recieved (plain  ):\n"); dumppacket(response); }

	// We have a challenge from the PIC here. It has supplied n+1 and given us P.
	if ( (response->seqnum != tosend.seqnum+1) )
	{
...
{
	printf("* SEQ number - 0x%08lx\n", mypacket->seqnum);
	printf("* Data bytes- 0x%02hhx 0x%02hhx 0x%02hhx 0x%02hhx\n", mypacket->nodeid,mypacket->byte6, mypacket->byte7, mypacket->byte8);	
	//	printf(" (%c:%c:%c:%c)\n", mypacket->nodeid, mypacket->byte6, mypacket->byte7, mypacket->byte8);
}
...
void decipher(unsigned int num_rounds, unsigned long* v, unsigned long* k, int vebosity) 
{
	...
	// todo: make PIC use same byte-ordering.
	v[0] = revLongByteOrder(v[0]);
	v[1] = revLongByteOrder(v[1]);
